...
PURPOSE : Check details on a tenancy , reset password, fecth in Excel compatible format, 
...
## -*- coding: utf-8 -*-
"""
Created on Fri Jul  6 14:18:39 2018
...
def main():
    ...
    if not basepath or basepath =='.':
        basepath=os.getcwd()
    basepath=os.path.join(basepath,'output/prov_template')
...
def default_values(config_file='',profile='',proxy='',debug_info='',output='',all_values=''):
    ...
    #setting proxy
    if proxy:
        os.environ["HTTP_PROXY"]='myproxy.server.com:80'
...
def tenancy_regiondetails(dict_config):
    ...
    #print("\n\n")
    #print("Tenancy Name",tenancy_name,sep='|')
    #print("\n\n")
    #print("Region|Home Region|Availability Domains")
    ...
    for eachregion in subscription_details:
        ...
        #print(eachregion.region_name,eachregion.is_home_region,' ; '.join(all_ADs[1:]),  sep='|')
        all_regions[eachregion.region_key]=all_ADs
        
    # printing compartment details
    #Hiddingforprovtemplate#print("\n\n")
    #Hiddingforprovtemplate#print("->Compartment Details")
    #Hiddingforprovtemplate#print("Compartment Name|Compartment OCID|Status|creation Time" )
    ...
    for eachcompartment in compartment_details:
        ...
        #Hiddingforprovtemplate#print(eachcompartment.name,eachcompartment.id,eachcompartment.lifecycle_state,eachcompartment.time_created,sep='|')
...
def list_compute(r_config):
    ...
    for compname,compid in all_active_compartments.items():
        ...
        for eachinstance in allvm_details:
            inst_name=str(eachinstance.display_name).replace('|',' ')
            inst_id=eachinstance.id
            inst_tag=str(eachinstance.defined_tags).replace(": {'Owner':","|").replace('{','').replace('}','')
            if not inst_tag: 
                inst_tag='|'
            elif '|' not in inst_tag:
                inst_tag=inst_tag+'|'
            ...
            if inst_lifecycle == "RUNNING":
                ...
                FQDNhost=str(FQDNhost.split('.')[0])
                inst_network=str(inst_vcns)+'/'+str(inst_subnets)
                inst_ad=str(inst_ad.split(':')[1])
                print("Compute",tenan_name,compname,inst_ad,inst_name,FQDNhost,inst_privips,inst_pubips, inst_shape,inst_tag,ocpu,memgb,sep='|')
            else:
                inst_status="NO Further DETAILS To SHOW as the instance is: "+inst_lifecycle
                print("Compute",tenan_name,compname,inst_ad,inst_name,inst_status,sep='|')
                #print("Compute",tenan_name,compname,inst_ad,inst_name,inst_status, " "," " , inst_shape,inst_tag,ocpu,memgb,sep='|')
                #print("Compute",tenan_name,compname,inst_ad,inst_name,inst_status ,inst_tag,compute_ocpu,compute_memgb,sep='|')
...
def list_compute_allregions(config):
    ...
    print("Type|TenancyName|Compartment Name|Availability Domain|Instance Name |hostname|PrivIPs|PubIPS|Shape|INST|Owner|ocpu|mem")
    for eachregion in all_regions.values():
        config['region']=eachregion[0]
        list_compute(config)
    #Hiddingforprovtemplate#print("Total OCPU      |", compute_ocpu)
    #Hiddingforprovtemplate#print("Total Memory GB |", compute_memgb)
...
def list_dbs(current_config):
    ...
    for compname,compid in all_active_compartments.items():
        ...
        for eachdb in db_system_details :
            db_name=str(eachdb.display_name).replace('|',' ')
            #
            db_domain=eachdb.domain
            db_tag=str(eachdb.defined_tags).replace(": {'Owner':","|").replace('{','').replace('}','')
            if not db_tag : db_tag='|'
            ...
            for eachvnic in db.list_db_nodes(compartment_id = compid,db_system_id = db_id, limit  =5000).data:
                ...
                if db_lifecycle == "AVAILABLE":
                    ...
                    #print("========================= ",db_ad)
                    try:
                     db_ad=str(db_ad.split(':')[1])
                    except:
                     pass
                    print("DB",tenan_name,compname,db_ad,db_name,db_hostname,IPS.private_ip,IPS.public_ip,db_shape,db_tag,ocpu,memgb,sep='|')
                else:
                    db_status="NO Further DETAILS To SHOW as the database is: "+db_lifecycle
                    #print("DB",tenan_name,compname,db_ad,db_name,db_hostname,db_status,db_tag,sep='|')
                    print("DB",tenan_name,compname,db_ad,db_name,db_hostname,db_status,sep='|')
                    #print("DB",tenan_name,compname,db_ad,db_name,db_hostname,db_status," "," ",db_tag,ocpu,memgb,sep='|')
...
def list_db_allregions(config):
    ...
    #print("Tier|Compartment Name | Region|Availability Domain|VCN/Subnet|DB System Name|DB FQDN|Priv IP|Pub IP|Shape|Status|DBNAME-UNQNAME-CHAR-NCHAR-PDBNAME-DBVERSION-LICENSE-DATASIZE")
    for eachregion in all_regions.values():
        config['region']=eachregion[0]
        list_dbs(config)
    #Hiddingforprovtemplate#print("Total OCPU      |", db_ocpu)
    #Hiddingforprovtemplate#print("Total Memory GB |", db_memgb)
    ...
    for compname,compid in all_active_compartments.items():
        ...
        for eachlbs in lb_details_data:
            ...
            if lbs_status == "ACTIVE":
                print("LBR",tenan_name,compname,lbs_region,lbs_name,lbs_shape,lbs_ip_address,lbaas_type,certs_deployed,sep='|')
            else:
                print("LBR",tenan_name,compname,lbs_region,lbs_name,lbs_shape,certs_deployed,lbs_ip_address,"The LBAAS may not be active",sep='|')
...
def load_balancers_all_region(current_config):
    ...
    #print("Tier|Compartment Name | Region|AD|VCN|LbaaS Name |Tags|IP Address|Pvt or Pub|Capacity|SSL")
    for eachregion in all_regions.values():
        config['region']=eachregion[0]
        ...
        for eachad in ad_in_this_region:
            ...
            if all_fss:
                for eachfss in all_fss:
                    ...
                    #print("FSS",tenan_name,compname,ad_name,fss_name,fss_status,export_paths,fss_GB,mounttarget_names,mounttarget_ips,fss_id,sep='|')
                    print("FSS",tenan_name,compname,ad_name,fss_name,fss_GB,export_paths,mounttarget_ips,sep='|')
...
def fss_all_regions(currentconfig):
    ...
    #print("Tier|Compartment Name |AD NAME|VCN|FSS NAME|FSS Status|export paths|FSS GB Used|Mount TargetNames|MOUNT IP|FSS OCID")
    for eachregion in all_regions.values():
        config['region']=eachregion[0]
        list_fss(config)
